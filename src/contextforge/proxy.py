...
class ProxyManager:
    ...
    async def bootstrap(self):
        ...
        if not servers:
            _log("⚠️  Proxy'lenecek sunucu bulunamadı.")
            return
        _log(f"🔧 {len(servers)} sunucu bulundu.")
        for server_def in servers:
            await self._connect(server_def)
        _log(f"✅ {len(self.registry.tools)} tool proxy'leniyor.")
    
    async def _connect(self, server_def):
        ...
        try:
            ...
            _log(f"   ✅ {name}: {len(result.tools)} tool")
